...
**Vault provider architecture** *(user decision 2026-07-11: non-plain vaults are provider modules)*
A `VaultProvider` trait + compile-time registry: each provider is a self-contained module declaring metadata (kind id, display name, config field schema — folder path / password / url+username+password), an unlock/connect flow, and the storage ops (scan/read/write/create/rename/move/delete/attachments; optional live-change feed). The vault switcher and creation UI render forms from provider metadata, so providers can be added/removed as single modules. `plain` stays built-in; `encrypted-db` (M14), `encrypted-files` (M15), `tinylord` (M16) are providers.

Build-time selection (public vs personal builds): every non-plain provider sits behind a Cargo feature (`provider-encrypted-db`, `provider-encrypted-files`, `provider-tinylord`; default = all on for personal builds). A public build compiles unwanted providers OUT of the binary. The frontend hardcodes nothing — `list_providers()` drives the vault-type picker and config forms, so an omitted provider leaves zero UI trace. A saved vault whose kind isn't in the build shows as "unsupported in this build" (never auto-removed).

**M14 — Encrypted vault type 1: SQLCipher container (`encrypted-db`)** *(feature_vault_upgrade branch; user decision 2026-07-11: BOTH encrypted types ship)*
Notes + attachments (blobs) + FTS index in ONE SQLCipher database. Live DB in app-data; the vault location holds `vault.jaynotes`, a consistent snapshot (`VACUUM INTO` + atomic rename) after each debounced edit burst. Not intended for Syncthing (that's M15) but degrades safely: external snapshot change → reload/import; `*.sync-conflict-*` sibling → import differing notes as visible "(conflict)" copies; deletion tombstones. This milestone also builds the SHARED encryption plumbing: VaultBackend abstraction (fs vs db), password-per-session prompt UI, optional cross-platform keyring remember (`keyring` crate), KDF layer shaped so FIDO2 hmac-secret/PRF passkey unlock slots in later. Images render from blobs.

**M15 — Encrypted vault type 2: rclone-crypt files (`encrypted-files`)** *(feature_vault_upgrade branch)*
Syncthing-friendly file-per-note encryption using local RcloneCryptRustLib (path dep; rclone-compatible: AES-EME names, XSalsa20-Poly1305 chunked content, scrypt KDF). Scan decrypts names (strays logged + skipped); reads stream-decrypt; writes encrypt_to temp + atomic rename; per-vault SQLCipher index (content must not leak to plaintext index); images via loopback decrypting proxy (crib GeoReel crypt_proxy); AI revisions stored inside the encrypted backing; ciphertext `*.sync-conflict-*` files name-decrypted and surfaced as openable conflict notes. Reuses M14's provider trait, password/keyring UX, and KDF layer.

**M16 — Vault type 3: TinyLord hosted (`tinylord`)** *(feature_vault_upgrade branch; user runs an instance, others can self-host)*
Provider config: base URL + username + password (per-instance). Auth via /v1/auth login/refresh (store refresh flow in the session; keyring opt-in same as encrypted vaults). Notes as documents in a collection (`{path, content, mtime, deleted}`), attachments as blob docs; doc CRUD maps onto the provider ops; server is source of truth (no Syncthing). REALTIME: SSE `/subscribe` feed keeps the local FTS index mirror and the open UI live across devices — an edit on one machine appears on the other within seconds. Offline behavior v1: read-only error state with reconnect (no offline queue yet). Multi-device conflict story: server-side last-writer-wins per document + optimistic-concurrency check if TinyLord exposes doc versions.
